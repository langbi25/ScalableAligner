...
void BF::set_bit(uint64_t p) {
            DEBUG: std::cout << "not yet implemented" ;
...
void BF::compress() {
    DEBUG: std::cout << "Cant compress rrr further with existing code base" ;
...
BF* UncompressedBF::union_with(const std::string & new_name, const BF* f2) const {
    ...
    if (b == nullptr) {
        DEBUG: std::cout << "Can only union two uncompressed BF" ;
...
uint64_t UncompressedBF::similarity(const BF* other, int type) const {
    ...
    if (o == nullptr) {
        DEBUG: std::cout << "Can only compute similarity on same type of BF." ;
	...
	// DEBUG: std::cout << "ERROR: ONLY TWO TYPES IMPLEMENTED" ;
...
BF* load_bf_from_file(const std::string & fn, HashPair hp, int nh) {
    if (fn.substr(fn.size()-4) == ".rrr") {
        ...
        DEBUG: std::cout << "unknown bloom filter filetype (make sure extension is .rrr or .bv)" ;
