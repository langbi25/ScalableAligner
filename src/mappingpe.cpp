...
AlignmentCandidate_t IdnetifyRescueCandidate(int64_t gPos, vector<SeedPair_t>& vec)
{
	...
	AlignmentCandidate.Score = s = 0; 
	AlignmentCandidate.PairedAlnCanIdx = -1;
...
bool RescueUnpairedAlignment(int EstDistance, ReadItem_t& r1, ReadItem_t& r2, vector<AlignmentCandidate_t>& AlignmentVec1, vector<AlignmentCandidate_t>& AlignmentVec2)
{
	...
	if (iFixStrategy == 1 || iFixStrategy == 3)
	{
		...
		thr = DetermineAnchorThreshold(AlignmentVec1); 
		vec1 = CreateKmerVecFromReadSeq(r2.rlen, r2.seq);
		for (j = num2, i = 0; i < num1; i++)
		{
			...
			iter = ChrLocMap.lower_bound(left_boundary); 
			chr_id = iter->second;
			if (right_boundary < GenomeSize && right_boundary > ChromosomeVec[chr_id].FowardLocation){
				right_boundary = ChromosomeVec[chr_id].FowardLocation - 1;
			} else if (right_boundary >= GenomeSize && right_boundary > ChromosomeVec[chr_id].ReverseLocation){
				right_boundary = ChromosomeVec[chr_id].ReverseLocation - 1;
			} 
			...
			RefSeg  = refSeq + left_boundary; 
			vec2 = CreateKmerVecFromReadSeq(slen, RefSeg);
	...
	if(iFixStrategy == 2 || iFixStrategy == 3){
		...
		for (i = num1, j = 0; j < num2; j++)
		{
			...
			RefSeg = refSeq + left_boundary; vec2 = CreateKmerVecFromReadSeq(slen, RefSeg);

			KmerPairVec = IdentifyCommonKmers(slen, vec1, vec2);
			SimplePairVec = GenerateSimplePairsFromCommonKmers(10, KmerPairVec);
			AlignmentCandidate = IdnetifyRescueCandidate(left_boundary, SimplePairVec);
			// fprintf(stdout,"left_boundary:%ld  right_boundary:%ld slen:%d\n",left_boundary,right_boundary ,right_boundary-left_boundary);
			// fprintf(stdout,"AlignmentCandidate.Score%d score1:%d\n",AlignmentCandidate.Score ,score1);
