	...
	if(file.is_open()){
		while(!file.eof())
		{
			...
			if (QueryMap.find(Readheader) != QueryMap.end()){
				...
				posInfo tmpPosInfo;
				if(Readheader.find_first_of('=')!=string::npos){
					PosInfoStr = Readheader.substr(Readheader.find_first_of('=')+1);
				} else{
					
					realChName = tmpReadheader.substr(0, tmpReadheader.find_first_of('_'));
					tmpReadheader =tmpReadheader.substr(tmpReadheader.find_first_of('_')+1);

					realPos1 =(unsigned int)atoi(tmpReadheader.substr(0, tmpReadheader.find_first_of('_')).c_str());
					tmpReadheader =tmpReadheader.substr(tmpReadheader.find_first_of('_')+1);		

					realPos2 =(unsigned int)atoi(tmpReadheader.substr(0, tmpReadheader.find_first_of('_')).c_str());

					tmpPosInfo.TrueChName =realChName;
					tmpPosInfo.TruePos1 =realPos1;
					tmpPosInfo.TruePos2 =realPos2;

				}	
				QueryMap[Readheader] =tmpPosInfo;	
			}
...
void Evaluation(string SamFileName ){
	...
	// precision, recall
	FILE *rfile = fopen("exp_1.csv", "a");
	if (iReadNum > 0){
		fprintf(rfile, "%s: %.2f\t%.2f\t%.2f\n", SamFileName.c_str(),(int)(10000*(1.0*(iReadNum-iUnmapped)/TotalQuery)+0.5)/100.0, (int)(10000*(1.0*iCorLocation/(iReadNum-iUnmapped))+0.5)/100.0, (int)(10000*(1.0*iCorLocation/TotalQuery)+0.5)/100.0);
